...
struct TsfnContext {
  ...
  TsfnContext(Napi::Env env) : deferred(Napi::Promise::Deferred::New(env)) { // 新知识点
  ...
  Napi::ThreadSafeFunction tsfn; // 新知识点
};
...
void threadEntry(TsfnContext *context) {
  ...
  for (size_t index = 0; index < ARRAY_LENGTH; ++index) {
    ...
    napi_status status =
        context->tsfn.BlockingCall(&context->ints[index], callback);  // 新知识点
